...
{context}
아래 콘텐츠 제목 목록 중, 위 과목과 명백하게 관련 없는 항목의 번호만 JSON 배열로 반환해.

[판정 기준 - 매우 엄격하게 적용]
- 관련 없음(❌): 과목과 완전히 다른 분야의 내용. 예) '프로그래밍기초2(C++)' 과목에 '딥러닝 모델 학습' 콘텐츠.
- 관련 있음(✅): 과목명이 직접 언급되거나, 과목에서 다루는 기술/개념/이론이 포함된 콘텐츠.
- 관련 있음(✅): 과목과 연관된 상위/하위 개념. 예) '운영체제' 과목에 '인터럽트', '프로세스 스케줄링' 내용.
- 관련 있음(✅): 과목의 도구/언어를 다루는 콘텐츠. 예) '컴퓨터비전' 과목에 'Python 영상처리'.
- 확실하지 않으면 반드시 관련 있음으로 판정할 것. 오탐(false positive)을 절대 내지 말 것.
