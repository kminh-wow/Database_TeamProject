...
위 대학 전공 과목에서 학부생이 중간/기말고사를 대비할 때 YouTube에서 검색할 법한 \
'구체적이고 고유한 학술 전문 용어' 3가지만 추출해. {syl_hint}

[절대 금지 규칙]
- '강의', '특강', '세미나', '캠페인', '프로젝트', '오리엔테이션', '수업' 같은 행위/행사 명사 금지.
- '컴퓨터', '시스템', '프로그램', '하드웨어', '소프트웨어' 같이 과목명 수준의 너무 광범위한 단어 금지.
- 반드시 해당 과목에서만 배우는 고유한 전문 개념어(예: 파이프라이닝, 캐시 메모리, 퀵정렬, 베이즈 정리)로 뽑을 것.
- 과목명 자체 포함 금지.
- 이수구분, 공학인증, 과목코드, 학점, 담당교수, 수강대상 등 행정/분류 정보는 절대 선택 금지.
